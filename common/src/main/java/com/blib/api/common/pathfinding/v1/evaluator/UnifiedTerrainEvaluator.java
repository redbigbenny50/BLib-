...
public final class UnifiedTerrainEvaluator implements TerrainEvaluator {
     ...
     * walkway (or at least a one-block gap from walls) whenever an only-slightly-longer lane exists. This is what stops
     * paths hugging the inner face of tunnel corners, where the entity's bounding box rubs the corner block, collision
     * keeps deflecting it off its waypoints, and near-equal inner-lane route ties flip on every repath — the
     * left/right/turn shuffle at bends. A "wall" is a cardinal neighbor solid at both feet and head height: two-high
     * obstruction. Floor steps, slabs, and other single-height terrain beside the node deliberately don't count, so
     * staircases and uneven ground aren't repelled. Doors the entity can pass (open, or closed while it can open doors)
     * don't count either — only a genuinely impassable closed door reads as wall, where repelling is correct.
     * Crawl-posture nodes are exempt at the call site (gap-crawl spaces are wall-tight by nature). Soft cost only —
     * narrow one-wide corridors remain fully traversable, and in uniform two-wide tunnels both lanes carry the same
     * malus so route choice is unaffected. The per-side cost comes from
